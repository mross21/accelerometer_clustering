...
def main():
    ...
    for file in all_files:
        ...
        for userAndWk, accGrp in dfAccByWk:
            ...
            accGrp['cluster'] = skm.fit_predict(accGrp[['x','y','z']])+1
            ...
            plt.xlim([-1.2,1.2])
            plt.ylim([-1.2,1.2])
            plt.savefig(out_path(PATH_FIGURES, 'user_' + str(int(user)) + '_week_' + str(int(wk)) + '_plot_skmeans-xz.png'))
        ...
        dfAccOut.to_parquet(out_path(PATH_CLUSTERS, 'user_'+str(int(user))+'_accel_withClusters-kmeans.parquet'))
