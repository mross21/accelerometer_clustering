...
for i in treeNodes: # iterate through the keys (i is key)
    ...
    for j in treeNodes[i].items(): # iterate through lists for each point j
        kList.append((i,j[1]['nodes'][-1],j[1]['densities'][-1])) # append last point in list (index of local max density)
    uniqueK = list(set(kList))
    # dfUnique = pd.DataFrame(uniqueK, columns=['node','density'])
...
for i in treeNodes:
    for j in list(treeNodes[i].items()):
        node_cluster.append((i,j[0],j[1]['nodes'][-1],j[1]['densities'][-1]))
        for n,d in zip(j[1]['nodes'],j[1]['densities']):
            node_list.append((i,j[0],n,d))

dfNodeCluster = pd.DataFrame(node_cluster, columns = ['userGroup','node','cluster','densityCluster'])
dfNodeMapping = pd.DataFrame(node_list, columns = ['userGroup','node','node_mapping','densityCluster'])
...
for usrGrp in dfNodeCluster['userGroup'].unique():
    # select rows for user/group pair
    grp2 = df.loc[df['userGroup'] == usrGrp].reset_index(drop=True)
    grpK = dfNodeCluster.loc[dfNodeCluster['userGroup'] == usrGrp].reset_index(drop=True)
    # get values
    xListN.append(grp2['x'].iloc[grpK['node']].reset_index(drop=True))
    yListN.append(grp2['y'].iloc[grpK['node']].reset_index(drop=True))
    zListN.append(grp2['z'].iloc[grpK['node']].reset_index(drop=True))
    thetaListN.append(grp2['theta'].iloc[grpK['node']].reset_index(drop=True))
    phiListN.append(grp2['phi'].iloc[grpK['node']].reset_index(drop=True))
    densityListN.append(grp2['density'].iloc[grpK['node']].reset_index(drop=True))
    xListC.append(grp2['x'].iloc[grpK['cluster']].reset_index(drop=True))
    yListC.append(grp2['y'].iloc[grpK['cluster']].reset_index(drop=True))
    zListC.append(grp2['z'].iloc[grpK['cluster']].reset_index(drop=True))
    thetaListC.append(grp2['theta'].iloc[grpK['cluster']].reset_index(drop=True))
    phiListC.append(grp2['phi'].iloc[grpK['cluster']].reset_index(drop=True))
    densityListC.append(grp2['density'].iloc[grpK['cluster']].reset_index(drop=True))

dfNodeCluster['xNode'] = flatten(xListN)
dfNodeCluster['yNode'] = flatten(yListN)
dfNodeCluster['zNode'] = flatten(zListN)
dfNodeCluster['thetaNode'] = flatten(thetaListN)
dfNodeCluster['phiNode'] = flatten(phiListN)
dfNodeCluster['densityNode'] = flatten(densityListN)
dfNodeCluster['xCluster'] = flatten(xListC)
dfNodeCluster['yCluster'] = flatten(yListC)
dfNodeCluster['zCluster'] = flatten(zListC)
dfNodeCluster['thetaCluster'] = flatten(thetaListC)
dfNodeCluster['phiCluster'] = flatten(phiListC)

dfNodeCluster.to_csv(pathOut + 'tree_node_cluster.csv', index=False)
dfNodeMapping.to_csv(pathOut + 'tree_node_mapping.csv', index=False)
