...
# redefine the axes again to fit the axis labels from the iOS data (z coming out of page)
        ...
        # filter densities less than 1/10 max density
        # dfDensities = dfDensities.loc[dfDensities['density'] >= max(dfDensities['density'])/10]
        ...
        plt.colorbar(p)
        # plt.show()
        plt.savefig(plotPath + '2D_plotXZ_10000pts_user_' + str(userAndWk[0]) + '_week_' + str(userAndWk[1]) + 'bw01.png')
        plt.close()
        plt.clf()
