...
# redefine the axes again to fit the axis labels from the iOS data (z coming out of page)
        ...
        ax.set_xlabel('X')
        ax.set_ylabel('Y')
        ax.scatter(dfDensities['x'], dfDensities['y'], c=d, s=50, cmap = 'viridis_r')
        ...
        ax.set_xlabel('Y')
        ax.set_ylabel('Z')
        ax.scatter(dfDensities['y'], dfDensities['z'], c=d, s=50,cmap = 'viridis_r')
        ...
        plt.savefig(plotPath + '2D_plots_user_' + str(userAndDay[0]) + '_day_' + str(userAndDay[1]) + '.png')
