...
def main():
    ...
    for file in all_files:
        ...
        grouping_list = list(dfAcc['weekNumber'].unique())
        x_list = []
        y_list = []
        for t in grouping_list:
            grp = dfAcc.loc[dfAcc['weekNumber'] == t].copy()
            grp['sessionNumByWk'] = grp.groupby(['sessionNumber'], as_index=False).ngroup()

            grpAgg = grp.groupby('hourOfWeek', as_index=False)[['date', 'hour', 'dateHour', 'hourOfWeek', 'sessionNumByWk',
            ...
            x_list.append(grpAgg['hourOfWeek'])
        ...
        try:
            ...
            plt.tight_layout()
            plt.savefig(out_path(PATH_FIGURES, 'user_' + str(int(user)) + '_allDays.png'))
            plt.close()
        except TypeError:
            continue
